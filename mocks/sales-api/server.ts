...
app.get("/documents", (req: Request, res: Response) => {
  ...
  if (dateFrom) {
    ...
    docs = docs.filter((d) => new Date(d.handoverDate).getTime() >= from);
  }
  if (dateTo) {
    const to = new Date(dateTo).getTime();
    docs = docs.filter((d) => new Date(d.handoverDate).getTime() <= to);
  }
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`Sales Mock API listening on port ${PORT}`);
  });
}

export { app, app as salesMockApp };
