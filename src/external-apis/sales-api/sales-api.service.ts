...
import type { SalesDocumentDto } from "./dto/sales-document.dto";
...
export interface SalesApiParams {
  ...
  dateFrom?: string;
  dateTo?: string;
  /** Local/mock only: asks upstream Sales mock to return 500 (integration tests). */
...
export class SalesApiService {
  ...
  async fetchDocuments(params: SalesApiParams): Promise<SalesDocumentDto[]> {
    ...
    if (params.mockForceError) {
      headers["x-force-error"] = "true";
    }

    this.logger.debug(
      `[SALES] Fetching vin=${params.vin} page=${params.page} pageSize=${params.pageSize}`,
    );
  ...
  private async httpGet(
    ...
    const response = await axios.get<{ documents: SalesDocumentDto[] }>(url, {
    ...
    return (
      ...
      (response.data as unknown as SalesDocumentDto[])
    );
  }
}
